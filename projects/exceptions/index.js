...
function calculator(number = 0) {
  ...
  return {
    sum: (...args) => args.reduce((prev, cur) => prev + cur, number),
    dif: (...args) => number - args.reduce((acc, item) => acc + item, 0),
    div: (...args) => {
      if (args.some((item) => item === 0)) {
        throw new Error('division by 0');
      }
      return args.reduce((acc, item) => acc / item, number);
    },
    mul: (...args) => args.reduce((acc, item) => acc * item, number),
  };
}
